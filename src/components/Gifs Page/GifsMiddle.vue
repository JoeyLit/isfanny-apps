<template>
  <div>
    <!-- Error alert -->
    <div>
      <ErrorAlert v-if="apiErrors === 'Failed to load gif memes'" />
    </div>
    <!-- Error alert -->

    <div v-if="allGifMemes.length > 0" class="middle__container">
      <div
        class="middle__meme--item animate__animated animate__slideInUp"
        v-for="(meme, index) in allGifMemes"
        :key="meme._id"
      >
    ...
    <div v-else-if="isFilteredMemes" class="middle__container">
      <div
        class="middle__meme--item animate__animated animate__slideInUp"
        v-for="(meme, index) in gifFilteredMemes"
        :key="meme._id"
      >
        <div class="publisher__profile" id="publisher__profile">
          <div class="left">
    ...
    <!-- Error alert -->
    <ErrorAlert v-if="apiErrors === 'Failed to load more gif memes'" />
    <!-- <Alert /> -->
...
export default {
  ...
  methods: {
    ...
    async loadMore() {
      ...
      let page = this.gif_pageNumber + 1;
      let meta = {
        page,
        // sort: "-createdAt",
        filter: this.filter,
  ...
  watch: {
    cat(newValue) {
      ...
      if (this.activateFilter === true) {
        this.clearGifsPageNumber();

        this.filter.category = newValue;
        this.setGifFilteredURL(this.filter);
...
.meme__cards--item-footer .truncate {
  width: 330px;
  white-space: nowrap;
  overflow: hidden;
